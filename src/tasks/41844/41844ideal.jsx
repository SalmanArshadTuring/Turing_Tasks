...
export default function App() {
  ...
  const finishGame = () => {
    const highestScoreTeam = teams.reduce((max, team) => (team.score > max.score ? team : max), teams[0]);
    setWinner(highestScoreTeam.name);
  };

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-4">Sports Scoreboard</h1>
      {winner && <h2 className="text-xl font-semibold mb-4">Winner: {winner}</h2>}
      ...
      <Button onClick={finishGame} className="mt-4" disabled={teams.length < 2}>
        Finish Game
      </Button>
    </div>
  );
