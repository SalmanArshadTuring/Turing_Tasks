...
const LeaveApprovalSystem = ({ requests, onApprove, onDeny }) => {
  return (
...
export default function App() {
  ...
  const updateLeaveBalance = (request) => {
    const days = (new Date(request.endDate) - new Date(request.startDate)) / (1000 * 60 * 60 * 24) + 1;
  ...
  return (
    <div className="container mx-auto p-4 space-y-8">
      ...
      <LeaveApprovalSystem requests={requests} onApprove={handleApprove} onDeny={handleDeny} />
    </div>
  );
}